...
$(document).ready(function() {
    ...
    $('#date').mask('9999-99-99 99:99').datetimepicker({
        currentText     :   '<?php _e('Hiện nay'); ?>',
        prevText        :   '<?php _e('Trước'); ?>',
        nextText        :   '<?php _e('Sau'); ?>',
        ...
        dayNamesMin     :   ['<?php _e('CN'); ?>', '<?php _e('2'); ?>', '<?php _e('3'); ?>', '<?php _e('4'); ?>',
            '<?php _e('5'); ?>', '<?php _e('6'); ?>', '<?php _e('7'); ?>'],
        closeText       :   '<?php _e('Kết thúc'); ?>',
        timeOnlyTitle   :   '<?php _e('Lựa chọn thời gian'); ?>',
    ...
    if (tags.length > 0) {
        ...
        ?>, {
            ...
            hintText        :   '<?php _e('Vui lòng nhập tên nhãn'); ?>',
            noResultsText   :   '<?php _e('Nhãn này không tồn tại, nhấn enter để tạo'); ?>',
            prePopulate     :   tagsPre,
...
<?php if ($options->autoSave): ?>
    ...
    $('#text').bind('input propertychange', function () {
        if (!locked) {
            autoSave.text('<?php _e('Chưa được lưu'); ?>' + (lastSaveTime ? ' (<?php _e('Lần lưu cuối cùng'); ?>: ' + lastSaveTime + ')' : ''));
        }
