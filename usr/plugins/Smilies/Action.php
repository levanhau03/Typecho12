...
class Smilies_Action extends Typecho_Widget implements Widget_Interface_Do
{
	...
	public function scanfolders()
	{
		...
		//菜单项目显示
		$opts = '<option value="none">'._t('Không có biểu tượng cảm xúc').'</option>';
		if ($lists) {
			$opts = '';
		...
		$set = $set ? $set : $curset;
		$grids = _t('Không tìm thấy biểu tượng cảm xúc nào trong thư mục %s','<span style="color:#467B96;">'.$set.'</span>');
		$extras = '';
		//排序表情显示
		if (isset($lists[$set])) {
			$list = $lists[''.$set.''];
			...
			foreach ($names as $name) {
				$grids .= '<div class="td" id="'.$name.'" title="'._t('Kéo tương ứng').'" style="cursor:move;"><img src="'.$option->pluginUrl.'/Smilies/'.$set.'/'.$name.'" alt=""/></div>';
			}
			$grids .= '</div>';
		}
		...
		if (isset($rests[$set])) {
			...
			if ($rest) {
				$extras .= '<div class="caption">------ &#8659; '._t('Thêm hình ảnh').' ------</div>';
			}
			foreach ($rest as $rname) {
				$extras .= '<div class="fix" title="'._t('Nhấp chuột phải để sao chép địa chỉ hình ảnh').'"><img src="'.$option->pluginUrl.'/Smilies/'.$set.'/'.$rname.'" alt=""/></div>';
			}
			$extras .= '</div>';
		}
