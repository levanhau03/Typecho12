...
?>
        ...
        <div class="comt-main" id="div-<?php $comments->theId(); ?>">
            ...
            <div class="comt-meta">
                <span class="comt-author"><?php echo $author; ?></span> <?php $comments->date('Y-m-d'); ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php $comments->reply('Trả lời'); ?>
            </div>
        </div>
...
<?php $this->comments()->to($comments); ?>
    <div class="title" id="comments">
        <h3>Bình luận <?php $this->commentsNum('<small></small>', '<b>1</b>', '<b>%d</b>'); ?></h3>
    </div>
    <div id="<?php $this->respondId(); ?>" class="no_webshot">
<?php if(!$this->allow('comment')){ ?>
        <h3 class="title">
            <strong>Bài viết bình luận đã được đóng lại!</strong>
        </h3>
<?php }else{ ?>
...
<?php endif; ?>
                   ...
                   <p><?php $comments->cancelReply('<font color="##FF5151"><br>Hủy</font>'); ?></p>
                </div>
                <div class="comt-box">
                    ...
                    <textarea placeholder="Chào mừng bạn đến bình luận ở đây" class="input-block-level comt-area" name="text" id="comment" cols="100%" rows="3" tabindex="1" ></textarea>
                    <div class="comt-ctrl">
                        <div class="comt-tips"></div>
                        <button type="submit" name="submit" id="submit" tabindex="5">Gửi</button>
...
<?php if(!$this->user->hasLogin()): ?>
                <div class="comt-comterinfo" id="comment-author-info" <?php if($this->remember('author',true) != "" && $this->remember('mail',true) != ""): ?>style="display:none;"<?php endif; ?> >
                    <ul>
                        <li class="form-inline"><label class="hide" for="author">Biệt hiệu</label><input class="ipt" type="text" name="author" id="author" value="<?php $this->remember('author'); ?>" tabindex="2" placeholder="Biệt hiệu (bắt buộc)" required><span class="text-muted"></span></li>
                        <li class="form-inline"><label class="hide" for="mail">Email</label><input class="ipt" type="text" name="mail" id="mail" value="<?php $this->remember('mail'); ?>" tabindex="3" placeholder="Email (bắt buộc)" <?php if ($this->options->commentsRequireMail): ?> required<?php endif; ?>><span class="text-muted"></span></li>
                        <li class="form-inline"><label class="hide" for="url">URL</label><input class="ipt" type="text" name="url" id="url" value="<?php $this->remember('url'); ?>" tabindex="4" placeholder="URL"><span class="text-muted"></span></li>
                    </ul>
                </div>
<?php endif; ?>
            </div>
