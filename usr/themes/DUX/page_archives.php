...
<?php
    ...
    if ($mon != $mon_tmp) {
    $mon = $mon_tmp;
    $output .= '<div class="item"><h3>'. $year . '/' . $mon .'</h3><ul class="archives-list">'; //输出月份
    }
    $output .= '<li><time>'.date('ngày d: ',$archives->created).'</time><a href="'.$archives->permalink .'">'. $archives->title .'</a> <span class="text-muted">'. $archives->commentsNum.' bình luận</span></li>'; //输出文章日期和标题
    endwhile;
