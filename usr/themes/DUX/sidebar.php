...
<?php if (!empty($this->options->sidebarBlock) && in_array('ShowRecentPosts', $this->options->sidebarBlock)): ?>
    <div class="widget widget-tops">
        <ul class="widget-nav"><li class="active">Bài mới</li></ul>
...
<?php if (!empty($this->options->sidebarBlock) && in_array('ShowCategory', $this->options->sidebarBlock)): ?>
    <?php
        ...
        for($i=1;$i<=$maxi;$i++){
            $icss=($i%2)+1;
            $tmp='<div class="widget widget_ui_textasb"><a class="style0'.$icss.'" href="'.$sad1[$i*3-3].'" target="_blank"><strong>Giới thiệu</strong><h2>'.$sad1[$i*3-2].'</h2><p>'.$sad1[$i*3-1].'</p></a></div>';
            echo $tmp;
        }
    ?>
<?php endif; ?>
...
<?php while($comments->next()): ?>
            <li>
            <a href="<?php $comments->permalink(); ?>" title="Bình luận tại <?php $comments->title(); ?>">
                <strong><?php $comments->author(false); ?></strong> (<?php $comments->date('m-d'); ?>) Nói:
