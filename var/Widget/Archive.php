...
(function () {
    ...
    private function singleHandle(Query $select, bool &$hasPushed)
    {
        ...
        if ($isPasswordPosted && $this->hidden) {
            throw new WidgetException(_t('Xin lỗi, mật khẩu bạn đã nhập sai'), 403);
        }
    ...
    private function categoryHandle(Query $select, bool &$hasPushed)
    {
        ...
        if (isset($directory) && ($this->request->directory != implode('/', $category['directory']))) {
            throw new WidgetException(_t('Chuyên mục không tồn tại'), 404);
        }
    ...
    private function tagHandle(Query $select, bool &$hasPushed)
    {
        ...
        if (!$tag) {
            throw new WidgetException(_t('Nhãn không tồn tại'), 404);
        }
    ...
    private function dateHandle(Query $select, &$hasPushed)
    {
        ...
        if (!empty($year) && !empty($month) && !empty($day)) {
            ...
            /** 设置标题 */
            $this->archiveTitle = _t('ngày %d tháng %d năm %d', $day, $month, $year);
            ...
            /** 设置标题 */
            $this->archiveTitle = _t('tháng %d năm %d', $month, $year);
        } elseif (!empty($year)) {
            ...
            /** 设置标题 */
            $this->archiveTitle = _t('năm %d', $year);
        }
