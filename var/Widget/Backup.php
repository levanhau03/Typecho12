...
class Backup extends BaseOptions implements ActionInterface
{
    ...
    private function import()
    {
        ...
        if (!empty($_FILES)) {
            ...
            if (0 == $file['error'] && is_uploaded_file($file['tmp_name'])) {
                ...
                Notice::alloc()->set(_t('Không tải lên được tệp sao lưu'), 'error');
                $this->response->goBack();
            }
        } else {
            if (!$this->request->is('file')) {
                Notice::alloc()->set(_t('Không có tệp sao lưu nào được chọn'), 'error');
                $this->response->goBack();
            }
            ...
            if (!file_exists($path)) {
                Notice::alloc()->set(_t('Tệp sao lưu không tồn tại'), 'error');
                $this->response->goBack();
            }
        }
    ...
    private function extractData($file)
    {
        ...
        if (!$fp) {
            Notice::alloc()->set(_t('Không thể đọc tệp sao lưu'), 'error');
            $this->response->goBack();
        }
        ...
        if ($fileSize < $headerSize) {
            @fclose($fp);
            Notice::alloc()->set(_t('Định dạng tệp sao lưu sai'), 'error');
            $this->response->goBack();
        }
        ...
        while (!feof($fp) && $offset + $headerSize < $fileSize) {
            ...
            if (!$data) {
                @fclose($fp);
                Notice::alloc()->set(_t('Lỗi khi khôi phục dữ liệu'), 'error');
                $this->response->goBack();
            }
        ...
        @fclose($fp);
        Notice::alloc()->set(_t('Khôi phục dữ liệu hoàn tất'), 'success');
        $this->response->goBack();
    }
    ...
    private function importData($table, $data)
    {
        ...
        try {
            ...
            Notice::alloc()->set(_t('Đã gặp các lỗi sau trong quá trình khôi phục: %s', $e->getMessage()), 'error');
            $this->response->goBack();
        }
    }
