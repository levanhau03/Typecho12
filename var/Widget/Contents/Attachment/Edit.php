...
class Edit extends PostEdit implements ActionInterface
{
    ...
    public function form(): Form
    {
        ...
        /** 提交按钮 */
        $submit = new Form\Element\Submit(null, null, _t('Gửi các thay đổi'));
    ...
    public function deleteAttachment()
    {
        ...
        if ($this->request->isAjax()) {
            $this->response->throwJson($deleteCount > 0 ? ['code' => 200, 'message' => _t('Tệp đã bị xóa')]
                : ['code' => 500, 'message' => _t('Không có tệp nào bị xóa')]);
        } else {
            /** 设置提示信息 */
            Notice::alloc()
                ->set(
                    $deleteCount > 0 ? _t('Tệp đã bị xóa') : _t('Không có tệp nào bị xóa'),
    ...
    public function clearAttachment()
    {
        ...
        /** 设置提示信息 */
        Notice::alloc()->set(
            $deleteCount > 0 ? _t('Các tệp không lưu trữ đã được dọn dẹp') : _t('Không có tệp nào chưa được lưu trữ đã được dọn dẹp'),
            $deleteCount > 0 ? 'success' : 'notice'
        );
