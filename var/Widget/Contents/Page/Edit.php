...
class Edit extends PostEdit implements ActionInterface
{
    ...
    public function markPage()
    {
        ...
        Notice::alloc()
            ->set(
                $markCount > 0 ? _t('Trang đã được đánh dấu là<strong>%s</strong>', $statusList[$status]) : _t('Không có trang nào được đánh dấu'),
    ...
    public function sortPage()
    {
        ...
        if (!$this->request->isAjax()) {
            ...
            $this->response->throwJson(['success' => 1, 'message' => _t('Việc sắp xếp trang đã được hoàn thành')]);
        }
    }
