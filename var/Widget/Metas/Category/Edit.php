...
class Edit extends Metas implements ActionInterface
{
    ...
    public function form(?string $action = null): Form
    {
        ...
        if ('insert' == $action || 'update' == $action) {
            ...
            $name->addRule([$this, 'nameToSlug'], _t('Không thể chuyển đổi tên danh mục thành tên viết tắt'));
        ...
        if ('update' == $action) {
            $mid->addRule('required', _t('Khóa chính phân loại không tồn tại'));
            $mid->addRule([$this, 'categoryExists'], _t('Phân loại không tồn tại'));
        }
    ...
    public function mergeCategory()
    {
        ...
        $validator->addRule('merge', 'required', _t('Khóa chính phân loại không tồn tại'));
        $validator->addRule('merge', [$this, 'categoryExists'], _t('Vui lòng chọn danh mục để hợp nhất'));
    ...
    public function sortCategory()
    {
        ...
        if (!$this->request->isAjax()) {
            ...
            $this->response->throwJson(['success' => 1, 'message' => _t('Sắp xếp đã hoàn thành')]);
        }
    }
    ...
    public function refreshCategory()
    {
        ...
        if ($categories) {
            ...
            Notice::alloc()->set(_t('Làm mới danh mục đã hoàn thành'), 'success');
        } else {
            Notice::alloc()->set(_t('Không có danh mục nào được chọn'), 'notice');
        }
