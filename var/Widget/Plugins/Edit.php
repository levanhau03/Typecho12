...
class Edit extends Options implements ActionInterface
{
    ...
    public function activate($pluginName)
    {
        ...
        if (Plugin::checkDependence($info['since'])) {
            ...
            if (
                ...
                throw new Exception(_t('Không thể bật plugin'), 500);
            }
            ...
            $result = _t('<a href="%s">%s</a> không hoạt động bình thường trong phiên bản Typecho này', $info['homepage'], $info['title']);
        }
    ...
    public function deactivate(string $pluginName)
    {
        ...
        if (isset($result) && is_string($result)) {
            ...
            Notice::alloc()->set(_t('Plugin đã bị vô hiệu hóa'), 'success');
        }
        $this->response->goBack();
    }
    ...
    public function config(string $pluginName)
    {
        ...
        if (!$this->configNoticed) {
            /** 提示信息 */
            Notice::alloc()->set(_t("Cài đặt plugin đã được lưu"), 'success');
        }
