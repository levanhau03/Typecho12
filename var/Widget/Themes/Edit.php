...
class Edit extends Options implements ActionInterface
{
    ...
    public function changeTheme(string $theme)
    {
        ...
        if (is_dir($this->options->themeFile($theme))) {
            ...
            Notice::alloc()->set(_t("Diện mạo đã thay đổi"), 'success');
            $this->response->goBack();
        } else {
            throw new Exception(_t('Kiểu bạn đã chọn không tồn tại'));
        }
    }
    ...
    public function editThemeFile($theme, $file)
    {
        ...
        if (
            ...
            if ($handle && fwrite($handle, $this->request->content)) {
                fclose($handle);
                Notice::alloc()->set(_t("Các thay đổi đối với tệp %s đã được lưu", $file), 'success');
            } else {
                Notice::alloc()->set(_t("Không thể ghi tệp %s", $file), 'error');
            }
            $this->response->goBack();
        } else {
            throw new Exception(_t('Tệp bạn đã chỉnh sửa không tồn tại'));
        }
    }
